...
const AnimatedItem: React.FC<AnimatedItemProps> = ({
  ...
  return (
    <motion.div
      ...
      className="flex items-center w-full gap-4 p-4 pl-0 md:pl-10 rounded-3xl shadow=sm hover:bg-[#cecece] hover:cursor-pointer"
    >
      {children}
    </motion.div>
  );
    ...
    <div className={`relative w-[500px] ${className}`}>
      ...
      >
        {items.map((item, index) => (
          ...
          >
            <div
              className={`flex items-center gap-4 p-4 pl-0 md:pl-10 rounded-3xl
                transition hover:bg-[#cecece] hover:cursor-pointer
                ${selectedIndex === index ? "bg-[#cecece]" : ""}
                ${itemClassName}
                `}
            >
