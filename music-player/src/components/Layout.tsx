...
const Layout = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      <div
        className={`fixed top-0 left-0 h-full z-50 sm:z-40 bg-[#b4b4b4] transition-transform duration-300 ${
          collapsed ? "-translate-x-full" : "translate-x-0"
        } w-[230px] sm:translate-x-0 sm:static sm:block`}
      >
        <SideBar collapsed={collapsed} setCollapsed={setCollapsed} />
      </div>

      <div
        className={`flex flex-col flex-1 bg-white shadow-2xl md:rounded-l-4xl overflow-hidden md:opacity-100 z-40 transition-all ${
          collapsed ? "opacity-100" : "opacity-10"
        }`}
      >
        <div className="flex-1 relative overflow-y-auto">
          {collapsed && (
            <button
              className="absolute top-6 left-4 z-50 text-2xl sm:hidden p-2 "
